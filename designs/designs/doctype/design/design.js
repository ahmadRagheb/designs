...
frappe.ui.form.on("Design", {
	...
	onload: function(frm) {
		var so = frappe.meta.get_docfield("Design", "sales_order");
		...
		frm.set_query("user", "users", function() {
			return {
				query:"designs.designs.doctype.design.design.get_users_for_project"
			}
		});
	...
	refresh: function(frm) {
		if(frm.doc.__islocal) {
			...
			frm.add_web_link("/designs?design=" + encodeURIComponent(frm.doc.name));

			if(frappe.model.can_read("Task")) {
				frm.add_custom_button(__("Gantt Chart"), function() {
					frappe.route_options = {"Design": frm.doc.name};
